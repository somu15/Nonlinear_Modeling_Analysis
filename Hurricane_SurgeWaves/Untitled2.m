names = importdata('C:\Users\lakshd5\Dropbox\Preliminary Analysis paper\Hurricanes\Wind data\.win files\Names.txt');
for jj = 1:38
    file_path = (strcat('C:\Users\lakshd5\Dropbox\Preliminary Analysis paper\Hurricanes\Wind data\.win files\',names{jj},'.win'));
req_lat = 30.1801;
 req_long = -85.8050;
 fid = fopen(file_path);
 info_line = fgetl(fid);
...
storms_u(jj) = mean(Wind_U);
storms_v(jj) = mean(Wind_V);
end
